...
JOB_ID="${1:-}"
PHASE="${2:-snapshot}"
KILLED_LEADER="${3:-unknown}"

PROJECT_ROOT="/home/ec2-user/gp"
EFS_ROOT="/mnt/efs/gp_artifacts"

if [ -z "$JOB_ID" ]; then
  echo "Usage: $0 <job_id> <phase> [killed_leader_container]"
  echo "Example: $0 job_xxx before_crash master2"
  exit 1
fi

RUN_DIR="$PROJECT_ROOT/logs/fault_tolerance/master_leader_crash/$JOB_ID"
SNAPSHOT_DIR="$RUN_DIR/$PHASE"

mkdir -p "$SNAPSHOT_DIR"

echo "[collect] test=master_leader_crash"
echo "[collect] job_id=$JOB_ID"
echo "[collect] phase=$PHASE"
echo "[collect] killed_leader=$KILLED_LEADER"
...
docker ps --format 'table {{.Names}}\t{{.Status}}\t{{.Ports}}' \
  > "$SNAPSHOT_DIR/docker_ps_compact.txt" 2>&1 || true
...
echo "[collect] Extracting leader/recovery evidence..."
{
  echo "=== Leader/recovery evidence ==="
  echo "phase=$PHASE"
  echo "killed_leader=$KILLED_LEADER"
  echo

  for log_file in "$SNAPSHOT_DIR"/container_logs/*.log; do
    if [ -f "$log_file" ]; then
      echo
      echo "----- $(basename "$log_file") -----"
      grep -Ei \
        "leader elected|this node is leader|not leader|election started|current leader|startup recovery|recovery|recoverable|workers available|resum|resume|failover|term|heartbeat|master leader|following leader|connected" \
        "$log_file" || true
    fi
  done
} > "$SNAPSHOT_DIR/leader_evidence.log"
...
{
  echo "fault_tolerance_test=master_leader_crash"
  echo "job_id=$JOB_ID"
  echo "phase=$PHASE"
  echo "killed_leader=$KILLED_LEADER"
  ...
  echo
  echo "docker_all_master_worker_containers:"
  docker ps -a --format 'table {{.Names}}\t{{.Status}}\t{{.Ports}}' | grep -E 'master|worker' || true
  echo
  ...
  if [ -f "$SNAPSHOT_DIR/efs_job/job_record.json" ]; then
    ...
    echo
    echo "job_record_status_lines:"
    grep -E '"status"|"model_id"|"message"|"selected_experiment_id"' \
      "$SNAPSHOT_DIR/efs_job/job_record.json" || true
  else
  ...
  echo
  echo "matching_manifests:"
  if [ -f "$SNAPSHOT_DIR/model/matching_manifests.txt" ]; then
    cat "$SNAPSHOT_DIR/model/matching_manifests.txt"
  fi
